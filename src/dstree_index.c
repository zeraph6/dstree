...
/**
 This function initializes the dstree_index_setting using the args,
 max_filename_size is size of some information used in to describe a leaf

 */
struct dstree_index_settings * dstree_index_settings_init(const char * root_directory,
						 unsigned int timeseries_size, 
						 unsigned int init_segments, // 1
						 unsigned int max_leaf_size,//100
						 ...
						 boolean is_index_new)//1
{
  ...
  if (is_index_new)
  {
    if(chdir(root_directory) == 0)
    {
        ...
        exit(-1);
    }//error if index filename already exists
  }
    ...
    */
     ...
     float segment_ends_size = ceil(log10(SHRT_MAX));//shrt_max is max value of short int, return x s.t : 10^x = 32767 // 2bytes for shrt int
     float split_value_size = ceil(log10(INT_MAX)+1);//int_max is max value of an int, 10^x = 2147483647 // 4bytes for int
...
/**
 This function initializes a dstree index by allocating space to dstree_index structure, init states and total record to 0, and  dstree_index_settings in index->setting
